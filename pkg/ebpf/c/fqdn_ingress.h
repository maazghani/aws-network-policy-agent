...
#ifdef FQDN_IPV6
    __builtin_memcpy(&legacy.saddr, packet->tuple.dst, 16);
    __builtin_memcpy(&legacy.daddr, packet->tuple.src, 16);
#else
    __builtin_memcpy(&legacy.src_ip, packet->tuple.dst, 4);
    __builtin_memcpy(&legacy.dest_ip, packet->tuple.src, 4);
#endif
    legacy.src_port = packet->tuple.dport;
    legacy.dest_port = packet->tuple.sport;
    ct = bpf_map_lookup_elem(&aws_conntrack_map, &legacy);
    if (ct) {
        ct->last_seen = bpf_ktime_get_ns();
        return BPF_OK;
    }
...
static __noinline int fqdn_ingress_reply(struct __sk_buff *skb, struct fqdn_endpoint *ep, struct fqdn_packet *packet)
{
    __u32 ifindex = skb->ifindex;
    struct fqdn_flow_key key = {.lifetime = ep->lifetime};
    fqdn_reverse_tuple(&key.tuple, &packet->tuple);
    __u64 now = bpf_ktime_get_boot_ns();
    if (key.tuple.dport == 53 && (key.tuple.protocol == IPPROTO_TCP || key.tuple.protocol == IPPROTO_UDP)) {
        __u32 config_key = 0;
        ...
        if (!config || !config->ready || !config->reply_mark ||
            ...
            return BPF_DROP;
        struct fqdn_dns_value *dns = bpf_map_lookup_elem(&fqdn_dns, &key.tuple);
...
static __noinline int fqdn_handle_ingress(struct __sk_buff *skb)
{
    ...
    if (packet.essential_icmp == 2)
        return BPF_OK;
    if (__builtin_memcmp(packet.tuple.dst, ep.address, 16))
        return BPF_DROP;
...
#endif
        return BPF_DROP;
    int result = fqdn_ingress_reply(skb, &ep, &packet);
